...
import com.dntr.registry.DntrMenus;
...
public class ArcaneSmithingMenu extends AbstractContainerMenu {
    ...
    public ArcaneSmithingMenu(int containerId, Inventory playerInventory, ArcaneSmithingTableBlockEntity entity) {
        super(DntrMenus.ARCANE_SMITHING.get(), containerId);
    ...
    private void tryBind() {
        ...
        if (spear.is(DntrItems.ARCANE_SPEAR_FOCUS.get()) && rune.is(DntrItems.RUNE.get())) {
            CompoundTag spearData = CustomDataUtil.getDntrData(spear);
            if (!spearData.contains("rune")) {
                CompoundTag runeData = CustomDataUtil.getDntrData(rune);
                if (runeData.contains("rune") && runeData.getCompound("rune").getInt("lvl") > 0) {
                    spearData.put("rune", runeData.getCompound("rune").copy());
                    CustomDataUtil.setDntrData(spear, spearData);
    ...
    private void tryUnbind() {
        ItemStack spear = blockEntity.inventory.getStackInSlot(5);
        if (spear.is(DntrItems.ARCANE_SPEAR_FOCUS.get())) {
            CompoundTag spearData = CustomDataUtil.getDntrData(spear);
            if (spearData.contains("rune")) {
                ...
                ItemStack outRune = new ItemStack(DntrItems.RUNE.get());
                CompoundTag rd = new CompoundTag();
                rd.put("rune", runeDataBody);
                CustomDataUtil.setDntrData(outRune, rd);
    ...
    private void tryRecharge() {
        ...
        CompoundTag data = CustomDataUtil.getDntrData(target);
        ...
        if (mat1.is(Items.GLOWSTONE_DUST) && mat1.getCount() >= lvl && mat2.is(Items.ECHO_SHARD)
            ...
            if (target.is(DntrItems.RUNE.get())) {
                CompoundTag wrap = new CompoundTag();
                wrap.put("rune", rune);
                CustomDataUtil.setDntrData(target, wrap);
            } else {
                data.put("rune", rune);
                CustomDataUtil.setDntrData(target, data);
            }
    ...
    private void tryApplyUpgrade() {
        ...
        if (runeStack.is(DntrItems.RUNE.get()) && !mat.isEmpty()) {
            CompoundTag dt = CustomDataUtil.getDntrData(runeStack);
            if (!dt.contains("rune"))
                return;
            ...
            if (rune.getInt("crude") == 0 && rune.getInt("empty") == 1) {
                ...
                if (upgType != null) {
                    ...
                    if ((curType.isEmpty() || curType.equals(upgType)) && curLevel < maxUpg) {
                        ...
                        dt.put("rune", rune);
                        CustomDataUtil.setDntrData(runeStack, dt);
                        mat.shrink(1);
                    }
                }
            }
    ...
    private void inscribe(int targetLvl) {
        ...
        if (runeStack.is(DntrItems.RUNE.get()) && !ink.isEmpty() && !tool.isEmpty()) {
            CompoundTag dt = CustomDataUtil.getDntrData(runeStack);
            if (!dt.contains("rune"))
                return;
            CompoundTag rune = dt.getCompound("rune");
            ...
            if (rune.getInt("crude") == 0 && rune.getInt("empty") == 1) {
                ...
                if (targetLvl == 1 && tool.is(DntrItems.SCROLL.get())) {
                    if (tier >= 1 && blockEntity.getTableLevel() >= 1) {
                        applyInscribe(rune, dt, runeStack, 1, spell);
                        tool.shrink(1);
                        ink.shrink(1);
                    }
                } else if (targetLvl == 2 && tool.getItem() == DntrItems.GRIMOIRE_T2.get()
                        || tool.getItem() == DntrItems.GRIMOIRE_T3.get()
                        || tool.getItem() == DntrItems.GRIMOIRE_T4.get()) {
                    // check tool tier
                ...
                } else if (targetLvl == 5 && tool.is(DntrItems.END_SIGIL.get())) {
                    if (tier >= 5 && blockEntity.getTableLevel() >= 5) {
                        applyInscribe(rune, dt, runeStack, 5, spell);
                        CompoundTag st = CustomDataUtil.getDntrData(tool);
                        int uses = st.getInt("uses") - 1;
                        if (uses <= 0)
                            tool.shrink(1);
                        else {
                            st.putInt("uses", uses);
                            CustomDataUtil.setDntrData(tool, st);
                        }
                        ink.shrink(1);
                    }
                }
    ...
    private void decreaseGrimoire(ItemStack tool) {
        CompoundTag gd = CustomDataUtil.getDntrData(tool);
        ...
        if (uses <= 0) {
            ...
            gd.putInt("uses", uses);
            CustomDataUtil.setDntrData(tool, gd);
        }
    }

    private int getGrimoireTier(ItemStack stack) {
        if (stack.is(DntrItems.GRIMOIRE_T1.get()))
            return 1;
        if (stack.is(DntrItems.GRIMOIRE_T2.get()))
            return 2;
        if (stack.is(DntrItems.GRIMOIRE_T3.get()))
            return 3;
        if (stack.is(DntrItems.GRIMOIRE_T4.get()))
            return 4;
        return 0;
    }

    private String determineSpellFromInk(ItemStack ink) {
        if (ink.is(DntrItems.INTENT_INK_FIRE.get()))
            return "fire";
        if (ink.is(DntrItems.INTENT_INK_STORM.get()))
            return "storm";
        if (ink.is(DntrItems.INTENT_INK_HEAL.get()))
            return "heal";
        if (ink.is(DntrItems.INTENT_INK_BREATHE.get()))
            return "breathe";
        if (ink.is(DntrItems.INTENT_INK_HEARTSTONE.get()))
            return "heartstone";
        if (ink.is(DntrItems.INTENT_INK_BREAKER.get()))
            return "breaker";
        if (ink.is(DntrItems.INTENT_INK_FERTILIZE.get()))
            return "fertilize";
        if (ink.is(DntrItems.INTENT_INK_WOLVES.get()))
            return "wolves";
        return null;
    }
}
